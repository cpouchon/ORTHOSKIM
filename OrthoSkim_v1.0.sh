        ...
        if grep -Fxq "${f}" ${RES}/nucleus_done.log; then
          ...
          if [ -s ${RES}/Mapping/nucleus/hits_${lib} ]; then
            ...
            if ${EXONERATE} --model protein2genome -q ${NUC_REF} -t ${RES}/Mapping/nucleus/contigs_hits_${lib}.fasta --showquerygff yes --showtargetgff yes --showvulgar no --showcigar no --showalignment no | awk '!/^Hostname:|^Command line:|^-- completed exonerate analysis|#/ {print $0}' > ${RES}/Mapping/nucleus/out_${lib}.gff; then
              echo "CMD: `dirname $0`/src/ExoGFF_threads.py -i ${RES}/Mapping/nucleus/contigs_hits_${lib}.fasta -g ${RES}/Mapping/nucleus/out_${lib}.gff -m ${mode} -o ${RES}/Extraction/${mode} -n ${lib} -l ${MINLENGTH} -t ${NUC_TYPE} --threads ${THREADS} -s ${NUC_REF}"
              `dirname $0`/src/ExoGFF_threads.py -i ${RES}/Mapping/nucleus/contigs_hits_${lib}.fasta -g ${RES}/Mapping/nucleus/out_${lib}.gff -m ${mode} -o ${RES}/Extraction/${mode} -n ${lib} -l ${MINLENGTH} -t ${NUC_TYPE} -c ${COVERAGE} -cl ${MINCONTLENGTH} --threads ${THREADS} -s ${NUC_REF}

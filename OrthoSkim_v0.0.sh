  ...
  if [ $mode == 'indexing' ]; then
    ...
    echo "CMD: ${DIAMOND} makedb --in ${NUC_REF} -d ${refdb} --threads ${THREADS}"
      ...
      if [ -s ${RES}/Mapping/nucleus/hits_${lib} ]; then
        ...
        `dirname $0`/src/ExoGFF.py -i ${RES}/Mapping/nucleus/contigs_hits_${lib}.fasta -g ${RES}/Mapping/nucleus/out_${lib}.gff -m ${mode} -o ${RES}/Extraction/ -n ${lib} -l ${MINLENGTH} -t ${NUC_TYPE} -c ${COVERAGE} -cl ${MINCONTLENGTH}
    ...
    echo "CMD: ${DIAMOND} makedb --in ${MITO_REF} -d ${refdb} --threads ${THREADS}"
      ...
      if [ -s ${RES}/Mapping/mitochondrion/hits_${lib} ]; then
        ...
        `dirname $0`/src/ExoGFF.py -i ${RES}/Mapping/mitochondrion/contigs_hits_${lib}.fasta -g ${RES}/Mapping/mitochondrion/out_${lib}.gff -m ${mode} -o ${RES}/Extraction/ -n ${lib} -l ${MINLENGTH} -t ${MITO_TYPE} -c ${COVERAGE} -cl ${MINCONTLENGTH}

...
if (($mode=='SPAdes_reformate')); then
  echo "OrthoSkim: preprocessing of assemblies files from SPAdes for nucleus/mitochondrion modes"
  for f in ${LIST_FILES};
  do
    samplename=`awk '{print $3}' $f`
    cp ${RES}/${PATHNAME_ASSEMBLY}/${samplename}/scaffolds.fasta ${RES}/${PATHNAME_ASSEMBLY}/Samples/${samplename}.fasta
  done
  echo "done"
fi

if (($mode=='SPAdes_assembly')); then
...
if (($mode=='nucleus')); then
  ...
  for f in ${RES}/${PATHNAME_ASSEMBLY}/Samples/*.fasta;
  do
    lib=${f/.fasta/}
    ${DIAMOND} blastx --outfmt 6 qseqid sseqid pident length mismatch gapopen qframe qstart qend sstart send evalue bitscore slen -d ${refdb} \
            -q $f -o ${RES}/nuc_mapping/matches_${lib} --evalue ${EVALUE}
...
if (($mode=='get_mitoRef')); then
  ...
  for file in `find ${RES}/mito_ref/refCDS/ -type f -name \*.fna`;
  do
...
if (($mode=='mitochondrion')); then
  ...
  do
    lib=${f/.fasta/}
    ${DIAMOND} blastx --outfmt 6 qseqid sseqid pident length mismatch gapopen qframe qstart qend sstart send evalue bitscore slen -d ${refdb} \
            -q $f -o ${RES}/mito_mapping/matches_${lib} --evalue ${EVALUE}
...
if (($mode=='stat_SPAdes')); then
  echo "OrthoSkim: Summary statistics of nuclear assemblies"
  ${QUAST} ${RES}/${PATHNAME_ASSEMBLY}/Samples/*fasta -o ${RES}/report_SPAdes_assemblies

    ...
    for taxa in taxa_dict:
        ...
        if taxaname in cur_genome.keys():
            if cur_genome[taxaname].description.split("; ")[2].split("=")[1]=="exon":
                rsiz=float(cur_genome[taxaname].description.split("; ")[6].split("=")[1])
                siz=int(cur_genome[taxaname].description.split("; ")[4].split("=")[1])
                type=str(cur_genome[taxaname].description.split("; ")[3].split("=")[1])
            else:
                rsiz=1.0
                siz=int(cur_genome[taxaname].description.split("; ")[4].split("=")[1])
                type=str(cur_genome[taxaname].description.split("; ")[3].split("=")[1])

...
parser.add_argument("-o","--outfile", help="outfile",
                    type=str)
parser.add_argument("--clean", help="option: clean provisional REFSEQ sequences",
                    action="store_true")
...
Bar = ProgressBar(len(cur_genome), 60, '\t checking annotations')
barp=0

bad_annot=list()
bad_annot_extended=list()
identical_annot=dict()
all_ids=dict()
...
for file in cur_genome:
    ...
    break_record=0
    if 'comment' in record.annotations and args.clean:
        if 'PROVISIONAL REFSEQ' in record.annotations['comment']:
            bad_annot.append(file)
            bad_annot_extended.append(file)
        else:
            pass
        if 'identical to' in record.annotations['comment']:
            ident=record.annotations['comment'].replace("\n"," ").split('identical to ')[1].split(".")[0]
            identical_annot[file]=ident
        else:
            pass
    if 'UNVERIFIED' in record.annotations['keywords']:
        bad_annot.append(file)
        bad_annot_extended.append(file)
    else:
        pass
    if 'partial genome' in record.description:
        bad_annot.append(file)
        bad_annot_extended.append(file)
    else:
        pass
...
for i in bad_annot:
    if i in list(identical_annot.keys()):
        bad_annot_extended.append(identical_annot[i])
    else:
        continue
...
for file in cur_genome:
    ...
    record=cur_genome[file]
    break_record=0
    if record.id in bad_annot_extended or record.name in bad_annot_extended:
        continue
    else:

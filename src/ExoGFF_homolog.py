...
def GeneExtraction(genenumber):
    ...
    num_exon=[]
    num_intron=[]
    geneid=list(besthits_filtered3.keys())[genenumber]
    extract_contigs=list()

    if len(besthits_filtered3[geneid])==1:
        refcoverage_dict={}
        refcoverage_pass=[]

        seqid = besthits_filtered3[geneid][0][1]
        concat=[]

        for hits in besthits_filtered3[geneid]:
            seqid=hits[1]
        ...
        if seqid in refcoverage_pass:
            ...
            if len(stored[besthits_filtered3[geneid][0]])==0:
                pass
            else:
                for parts in stored[besthits_filtered3[geneid][0]]:
                ...
                num_exon.append(str(stat_stored[besthits_filtered3[geneid][0]]["exon"]))
                num_intron.append(str(stat_stored[besthits_filtered3[geneid][0]]["intron"]))
        ...
        for idx in orderindx:
            seqid = besthits_filtered3[geneid][idx][1]
            if seqid in refcoverage_pass:
                ...
                list_combo=list()
                pos_to_extract=list()
                if len(stored[besthits_filtered3[geneid][idx]])==0:
                    pass
                else:
                    for parts in stored[besthits_filtered3[geneid][idx]]:
                        if 'frame' in parts.keys():
                            fr=parts['frame']
                        else:
                            start=int(parts['min'])
                ...
                num_exon.append(str(stat_stored[besthits_filtered3[geneid][idx]]["exon"]))
                num_intron.append(str(stat_stored[besthits_filtered3[geneid][idx]]["intron"]))
            else:
                continue
...
if len(list(besthits_filtered3.keys()))>0:
    print ("Extraction of sequences for %s" % nameofsample)
    inputs=range(len(list(besthits_filtered3.keys())))

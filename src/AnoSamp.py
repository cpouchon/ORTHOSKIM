...
if model in ("chloroplast","nucrdna","mitochondrion"):
    ...
    Bar = ProgressBar(sum(value <2 for value in dict_genes.values()), 60, '\t Extraction of sequences for %s' % file_name_without_extension)
    barp=0
    for g in genes:
        g_tab = g.rstrip().split('\t')
        cur_genome = SeqIO.parse(f, formatin)
                            ...
                            if (gene == g_tab[1]) or (gene == g_tab[1]+"_1"):
                                ...
                                if feat.type == "tRNA" :
                                        ...
                                        if os.path.isfile(os.path.join(outpath+"/"+model+'_'+typeofgene, fname)):
                                            with open(os.path.join(outpath+"/"+model+'_'+typeofgene, fname), 'a+') as file:
                                                ...
                                                if not header in old_headers:
                                                    #print "extraction of %s for %s in %s" % (g_tab[1],f,os.path.join(model+'_'+typeofgene, fname))
                                                    Bar.update(barp)
                                                    file.seek(end_file)
                                                    file.write(header+'\n')
                                                    file.write(str(out_seq)+'\n')
                                                else:
                                                    #print "%s already found in %s" % (f,os.path.join(model+'_'+typeofgene, fname))
                                                    pass
                                        else :
                                    ...
                                    if dict_genes[gene]<2:
                                        ...
                                        if 'translation' in feat.qualifiers:
                                            if os.path.isfile(os.path.join(outpath+"/"+model+'_'+typeofgene, pfname)):
                                                with open(os.path.join(outpath+"/"+model+'_'+typeofgene, pfname), 'a+') as file:
                                                    old_headers = []
                                                    end_file=file.tell()
                                                    file.seek(0)
                                                    for line in file:
                                                    ...
                                                    if not header in old_headers:
                                                        #print "extraction of %s for %s in %s" % (g_tab[1],f,os.path.join(model+'_'+typeofgene, pfname))
                                                        file.seek(end_file)
                                                        file.write(header+'\n')
                                                        file.write(str(pseq)+'\n')
                                                    else:
                                                        #print "%s already found in %s" % (f,os.path.join(model+'_'+typeofgene, pfname))
                                                        pass
                                            else :
                                                with open(os.path.join(outpath+"/"+model+'_'+typeofgene, pfname), 'a') as out:
                                                    #print "extraction of %s for %s in %s" % (g_tab[1],f,os.path.join(model+'_'+typeofgene, pfname))
                                                    out.write(header+'\n')
                                                    out.write(str(pseq)+'\n')

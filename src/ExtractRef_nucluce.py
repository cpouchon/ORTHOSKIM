...
parser.add_argument("--target", help="gene type targeted",
                    type=str,choices=["uce","nucleus_aa","nucleus_nt"])
    ...
    if len(ncbi.get_taxid_translator([int(query)]))==0:
        if len(ncbi.get_name_translator([" ".join(query_name.split("_")[0:2])]))==0:
            if len(ncbi.get_name_translator([" ".join(query_name.split("_")[0:1])]))==0:
                ...
                print("WARN: Error on query taxid: %s, the longest sequence will be extracted for each gene." % str(query_name))
            else:
...
for g in stored.keys():
    ...
    if model=="taxonomy":
        if query_error>0:
            #pb with the query
            #so we extract the longest sequence
            lengthlist=list()
            namelist=list()
            seqlist=list()
            ...
            out_seq=str(seqlist[suborderindx[0]])
                        ...
                        if len(tree.search_nodes(name=str(query)))==0:
                            if len(ncbi.get_lineage(query))>0:
                                ...
                                if len(tree.search_nodes(name=str(search_taxid)))>0:
                                    ...
                                    #condition to extract the longest sequence by gene
                                    lengthlist=list()
                                    namelist=list()
                                    seqlist=list()
                                    seqlist=[b['seq'] for c in stored[gene_id].values() for b in c]

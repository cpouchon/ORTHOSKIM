...
for b in list_barcodes:
    ...
    with open(str(outpath+"/"+"blast_"+b+".out")) as out:
        for l in out:
            ...
            sub_name=tab[1].replace(str(b+"_"),"")
            try:
                sub_taxid=[int(i) for i in sub_name.split("_") if i.isdigit()][0]
            except:
                continue
            sub_split=sub_name.replace(str(str(sub_taxid)+"_"),"").split("_")
            sub_sp="_".join(sub_split[0:2])
            query_split=query_name.split("_")[0:2]
            query_sp="_".join(query_split[0:2])
            score=tab[2]
